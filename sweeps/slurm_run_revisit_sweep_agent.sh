...
#SBATCH --job-name=REVISIT_SWEEP_AGENT
...
# Usage: sbatch sweeps/slurm_run_revisit_sweep_agent.sh [SWEEP_ID] [ENTITY] [PROJECT]
# Example: sbatch sweeps/slurm_run_revisit_sweep_agent.sh abc123def myentity myproject
...
SWEEP_ID=${1:-""}
ENTITY=${2:-""}
PROJECT=${3:-"dna-embedding-revisit"}
...
if [ -z "$SWEEP_ID" ]; then
    ...
    LATEST_INIT=$(ls -t INIT_REVISIT_SWEEP_*.out ${BASEFOLDER}/sweeps/INIT_REVISIT_SWEEP_*.out 2>/dev/null | head -1)
...
if [ -z "$SWEEP_ID" ]; then
    ...
    echo "Usage: sbatch sweeps/slurm_run_revisit_sweep_agent.sh [SWEEP_ID] [ENTITY] [PROJECT]"
    echo "Or ensure INIT_REVISIT_SWEEP job has completed and output file exists."
    exit 1
fi
