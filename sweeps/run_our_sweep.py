...
Usage:
    python sweeps/run_our_sweep.py [--sweep-config sweeps/our_sweep.yaml] [--project PROJECT_NAME] [--entity ENTITY_NAME] [--count COUNT]
...
def initialize_sweep(config_path: str, project: str = None, entity: str = None) -> str:
    ...
    print(
        f"View sweep at: https://wandb.ai/{entity or wandb.api.viewer()['entity']}/{project or 'dna-embedding-our'}/sweeps/{sweep_id}"
    )
...
def main():
    ...
    if not args.init_only:
        ...
        print(
            f"  wandb agent {args.entity or ''}/{args.project or 'dna-embedding-our'}/{sweep_id}"
        )
